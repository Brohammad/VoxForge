...
**Impact:** Recruiters and pilot customers can try a live demo without cloning. Engineering evaluators can inspect ADRs, runbooks, case study, and 412+ tests.
...
- **Established engineering quality bar** with 412+ automated tests (404 pytest + 8 Playwright), 81% code coverage, dependency audit, and secret scanning in CI.
...
- Built **VoxForge**, open-source voice AI platform (Python/FastAPI, LangGraph, pgvector) — live in prod with HTTPS, 412+ tests, operator dashboard.
...
- **A:** 412+ tests, Playwright browser suite, smoke scripts, gitleaks/pip-audit in CI, public deployment.
- **R:** Green CI, live demo, GitHub release — portfolio-ready artifact.

### Story 4 — Pilot hardening under real auth flows

- **S:** RC-1 shipped, but PM review found cookie sessions broke dashboard nav, SAML cold-login failed, and policy presets did not affect live calls.
- **T:** Close gaps before calling the pilot “production-ready.”
- **A:** Fixed `isAuthenticated()` gating, wired active agent config into `VoicePipelineService`, added knowledge list/delete APIs, restructured dashboard IA, and expanded CI (format, browser, rate-limit fixes).
- **R:** Merged 27-commit PR with green CI; demo GIF automated; [case study](./case-study.md) documents the arc for interviews.
...
| Metric | Value |
|--------|-------|
| Automated tests | 412+ (404 pytest + 8 Playwright) |
| Code coverage | 81% |
...
In my personal project VoxForge, I set out to prove I can ship production systems—not prototypes. I designed and deployed an open-source voice AI platform with a unified pipeline across WebSocket and WebRTC, knowledge retrieval with pgvector, per-turn evaluation, and operator tooling including replay and human handoff. The system is live at voxforge.brohammad.tech with automated HTTPS deployment, 412+ tests, and CI gates for coverage, dependency audit, and secret scanning. It demonstrates the same skills I bring to product engineering: clear architecture, operational maturity, and user trust through observability and documentation.
